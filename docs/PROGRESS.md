...
| **4 · Home + Today + Daily Plan + Energy** | 🟡 partial | `localPlanner` (deterministic, 9 tests) + `NowNextCard` + `RemainingTodayCard` on Home shipped. Remaining: `daily_plans` / `daily_energy` tables + persist `/plan` + history + energy check-in + deterministic "أنا متأخر". |
