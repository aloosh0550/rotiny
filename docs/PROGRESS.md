...
- `tsc` clean · `lint` clean · `vitest` **60 pass / 8 integration skipped in CI**
- `next build` (cloud + `build:local`) clean · `cap sync android` clean
- Playwright QA (`build:local`): **28/28** screens, 360/393, RTL + dark, no overflow
- **Local Supabase integration** (`RUN_SUPABASE_INTEGRATION=1 npm run test:integration`): 8/8
  ...
  4b. Phase-5 task fields → round-trip · 4c. measurements increment + Arabic unit → round-trip
...
- **Hosted project** `qyrxtacuxojpdpujzjah`: migrations `20260906120000`, `20260906130000`,
  ...
  `20260908000000_phase5_task_fields` **applied** (owner-confirmed). **Pending: apply `20260909000000_phase6_measurements.sql`** (additive, verified on the local stack).
